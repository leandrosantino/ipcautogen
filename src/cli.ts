...
const config: Config = JSON.parse(fs.readFileSync(path.join(dirname, "ipcgenconfig.json"), {
  encoding: 'utf-8'
}).toString() || '{}')
...
const sourceFiles = project.addSourceFilesAtPaths(path.join(dirname, config.sourceDir) + "/**/*.ts")
...
//   path.join(dirname, config.outputDir, `/ipcInvoke.ts`),
//   script,
//   { overwrite: true }
// )
